...
@Reducer
struct RemindersReducer {
	@Dependency(\.remindersService) private var remindersService
...
// MARK: - ViewActions
private extension RemindersReducer {
	func viewActions(_ action: Action.ViewAction, state: inout State) -> Effect<Action> {
		switch action {
		case .onAppear:
			return .run { send in
				await send(.internal(.permissionLoaded(remindersService.checkPermission())))
			}
		...
		case .onSyncTapped:
			...
			return .run { _ in await remindersService.requestAccess() }

		case .onSyncConfirmed:
			state.showSyncDialog = false
			return .run { _ in await remindersService.sync() }
		...
		case .onClearConfirmed:
			...
			return .run { _ in await remindersService.clear() }
